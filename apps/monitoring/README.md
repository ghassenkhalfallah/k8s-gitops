...
- **Access:** Grafana is exposed via the existing NGINX Ingress Controller (host `grafana.k8s.local`, no new NodePort) — reach it the same way as everything else in this cluster, through a bastion SSH tunnel to NodePort 30080. Prometheus itself has no external exposure; Grafana reaches it over the in-cluster Service DNS via the chart's auto-provisioned datasource.
